...
`git add` 후 `gitmsg` 한 번으로 AI가 커밋 메시지를 작성하고 커밋까지 완료해주는 CLI 도구.
...
`install.sh`가 하는 일:
1. Docker 설치 여부 확인
2. Docker 이미지(`gitmsg-tool`) 빌드
3. `~/.zshrc`에 `gitmsg` 함수 추가 (중복 추가 방지)
...
| 상황 | 예시 |
|------|------|
| 일반 레포 / 단일 Next.js 앱 | `feat: 회원가입 기능 추가` |
...
```
gitmsg
 └─ docker run -v $(pwd):/workspace gitmsg-tool   # diff 분석 + 메시지 생성
...
`gitmsg.py` 수정 후에는 이미지를 재빌드해야 반영됩니다.
